            ...
            switch (neoRADIO2_state)
            {
                ...
                case DeviceInit:
                {
                    ...
                    if(Devices > 0)
                    {
                        for (int i = 0; i < Devices; i++)
                        {
                            ...
                            if(deviceInfo[i].State != neoRADIO2state_Connected && timeout == 0 && Devices == 1)
                            {
                                deviceConnected = false;
                ...
                case SendSettings:
                {
                    if(result == 0)
                    {
                        try
                        {
                            ...
                            neoRADIO2SetSettings(&deviceInfo[usbIndex]);
                ...
                case CalInteractive:
                {
                    ...
                    if (result == 0 && !closed())
                    {
                        ...
                        if(neoRADIO2returnCalibrationDataJSON(&deviceInfo[usbIndex], &sample, &messageData) == 0)
                        {
                            std::this_thread::sleep_for(std::chrono::milliseconds(1));
                ...
                case SetPwrRly:
                {
                    ...
                    if(result == 0)
                    {
                        ...
                        int returnValue = neoRADIO2SetPwrRly(&deviceInfo[usbIndex], &messageData);
                    }
                    neoRADIO2_state = DeviceIdle;
                }
                    break;
