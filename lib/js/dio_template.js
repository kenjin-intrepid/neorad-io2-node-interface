        ...
        return `
            <div class="form-group">
                ...
                <div class="input-group input-group-sm mb-3 opacity">
                    <input class="form-control form-control-sm din_ch${number}_value" placeholder="Ch ${number}" disabled>
                    <div class="input-group-prepend">
                        <label class="input-group-text form-control-sm ch${number}_unit_label"></label>
                    </div>
                </div>
            </div>
        `
    ...
    for (let i = 1; i < 9; i++)
    {
        ...
        if(i < 5)
        {
            ...
            for(let j = 1; j < 4; j++)
            {
                ...
                if(j != 3)
                {
                    din_channel_template_holder += `<hr>`;
                }
            }
        ...
        else
        {
            ...
            temp_template += `
                <td class="device_channel${i} em1" data-device="ma_device_c${i}" data-bank="${i}">
                    <div class="dout_group">
                        ...
                        <div class="form-group">
                            ...
                            <div class="ch1_dout_input_group hidden">
                                <label class="ch1_dout_label"></label>
                                <div class="input-group input-group-sm mb-3 opacity">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control form-control-sm dout_state_select dout_ch1_select" data-deviceindex="${index}" data-bankindex="${i}" disabled>
                                <option value="0">HIZ</option>
                                <option value="1">Low</option>
                                <option value="2">High</option>
                            </select>
                            <select class="form-control form-control-sm dout_state_select dout_ch1_h_select hidden" data-deviceindex="${index}" data-bankindex="${i}" disabled>
                                <option value="0">HIZ</option>
                    ...
                    <hr>
                    <div class="dout_group dout_ch2_group">
                        <div class="form-group">
                            ...
                            <div class="ch2_dout_input_group hidden">
                                <label class="ch2_dout_label"></label>
                                <div class="input-group input-group-sm mb-3 opacity">
                        ...
                        <div class="form-group">
                            ...
                            <select class="form-control form-control-sm dout_state_select dout_ch2_select" id="dout_ch2_select${index}_${i}" data-deviceindex="${index}" data-bankindex="${i}" disabled>
                                <option value="0">HIZ</option>
                                <option value="1">Low</option>
                                <option value="2">High</option>
                            </select>
                    ...
                    <button type="button" class="btn btn-info dout_set_apply" disabled>Apply</button>
                </td>
                `;
        }
...
dio_template.events = function(){
    ...
    $(".ma_device").on('change', '.dout_mode_select',function () {
        ...
        switch ($(this).val()) {
            case "0":
                ...
                $(this).parents('.ma_device_wrap').find(`.device_channel${which_bank} .ch${which_channel}_dout_label`).html('');
                $(this).parents('.ma_device_wrap').find(`.device_channel${which_bank} .ch${which_channel}_dout_unit_label`).html('');

                $(this).parents('.dout_group').find(`.dout_state_select`).val('0');
                $(this).parents('.dout_group').find(`.dout_state_select`).prop('disabled',true);
                break;
            case "1":
                $(this).parents('.ma_device_wrap').find(`.device_channel${which_bank} .ch${which_channel}_dout_input_group`).addClass('hidden');
                $(this).parents('.ma_device_wrap').find(`.device_channel${which_bank} .ch${which_channel}_dout_label`).html('');
                ...
                $(this).parents('.dout_group').find(`.dout_state_select`).prop('disabled',false);
                break;
            case "2":
                ...
                $(this).parents('.ma_device_wrap').find(`.device_channel${which_bank} .ch${which_channel}_dout_label`).html('Duty');
                $(this).parents('.ma_device_wrap').find(`.device_channel${which_bank} .ch${which_channel}_dout_unit_label`).html('%');
    ...
    $(".ma_device").on('change', '.device_hb_check_status',function () {
        ...
        if(value)
        {
            ...
            $(this).parents('.ma_device_wrap').find(`.device_channel${bank_value} .dout_ch1_h_select`).removeClass('hidden');
            $(this).parents('.ma_device_wrap').find(`.device_channel${bank_value} .dout_ch1_select`).addClass('hidden');
            $(this).parents('.ma_device_wrap').find(`.device_channel${bank_value} .dout_ch1_h_select`).val($(this).parents('.ma_device_wrap').find(`.device_channel${bank_value} .dout_ch1_select`).val());
        }
        else
        {
            ...
            $(this).parents('.ma_device_wrap').find(`.device_channel${bank_value} .dout_ch1_h_select`).addClass('hidden');
            $(this).parents('.ma_device_wrap').find(`.device_channel${bank_value} .dout_ch1_select`).removeClass('hidden');
            let val = $(this).parents('.ma_device_wrap').find(`.device_channel${bank_value} .dout_ch1_h_select`).val();
            if(val == 3)
            {
                val = 0;
            }
            $(this).parents('.ma_device_wrap').find(`.device_channel${bank_value} .dout_ch1_select`).val(val);
        }
    });
    ...
    $('.ma_device').on('click','.dout_set_apply',function(){
        ...
        let state1_h = parseInt($(that).find(`.device_channel${bank} .dout_ch1_h_select`).val());
        ...
        // 2 = pwm freq
        // 3 = state
        eachBankSettings.channel1 = [mode1, isNaN(data1) ? 0 : data1, freq1, h_mode ? state1_h : state1];
