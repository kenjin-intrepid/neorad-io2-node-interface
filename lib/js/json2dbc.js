...
function generateTemplate(sortobj, obj){
    ...
    if(Array.isArray(obj))
    {
        for(let i = 0; i < arb_index_length; i++)
        {
            ...
            let bank_main = "";
            objdata["Tagname"] ? bank_main = objdata["Tagname"] : bank_main = `Bank${Object.values(sortobj["arbid_index"])[i] + 1}`;
            ...
            switch (obj[obj_index].type) {
                case 0:
                    ...
                    SG_ = ` SG_ ${bank_main} : ${Bytestart}|32@1+ (1,0) [${device_description.tc_min_max}] "${device_description.temp_unit}" \r`;
                    break;
                case 2:
                    description = device_description.pwrrly;
                    SG_ = ` SG_ ${bank_main} : ${Bytestart}|32@1+ (1,0) [${device_description.rly_min_max}] "${device_description.power_unit}" \r`;
                    break;
                case 3:
                    description = device_description.ain;
                    SG_ = ` SG_ ${bank_main} : ${Bytestart}|32@1+ (1,0) [${device_description.ain_min_max}] "${device_description.power_unit}" \r`;
                    break;
                case 4:
                    description = device_description.aout;
                    SG_ = ` SG_ ${bank_main} : ${Bytestart}|32@1+ (1,0) [${device_description.aout_min_max}] "${device_description.power_unit}" \r`;
                    break;
            }

            template += `BO_ ${arbid} ${description}: ${byte_size} \r`;
            template += SG_;
            ...
            for(let j = 0; j < sg_index_length; j++)
            {
                ...
                let bank_sub = "";
                sub_objdata["Tagname"] ? bank_sub = sub_objdata["Tagname"] : bank_sub = `Bank${Object.values(sortobj["arbid_index"])[i] + 1}`;
                let bytestart = sub_objdata["ByteStart"] * 8;

                switch (obj[sub_obj_index].type) {
                    case 0:
                        SG_ = ` SG_ ${bank_sub} : ${bytestart}|32@1+ (1,0) [${device_description.tc_min_max}] "${device_description.temp_unit}" \r`;
                        break;
                    case 3:
                        SG_ = ` SG_ ${bank_sub} : ${bytestart}|32@1+ (1,0) [${device_description.ain_min_max}] "${device_description.power_unit}" \r`;
                        break;
                    case 4:
                        SG_ = ` SG_ ${bank_sub} : ${bytestart}|32@1+ (1,0) [${device_description.aout_min_max}] "${device_description.power_unit}" \r`;
                        break;
                }
