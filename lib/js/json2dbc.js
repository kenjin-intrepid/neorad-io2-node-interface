...
function generateTemplate(sortobj, obj){
    ...
    if(Array.isArray(obj))
    {
        for(let i = 0; i < arb_index_length; i++)
        {
            ...
            objdata["Tagname"] ? bank_main = objdata["Tagname"] : bank_main = `${description}_Bank${bank_main_value}`;
            SG_ = ` SG_ ${bank_main} : ${Bytestart}|32@1+ (1,0) [${description2}] "${description3}" Vector__XXX\n`;
            template += `BO_ ${arbid} ${description}_${obj[obj_index]['deviceID']}: ${byte_size} Vector__XXX\n`;
            template += SG_;
            ...
            for(let j = 0; j < sg_index_length; j++)
            {
                ...
                SG_ = ` SG_ ${bank_sub} : ${bytestart}|32@1+ (1,0) [${description2}] "${description3}" Vector__XXX\n`;
                template += SG_;
