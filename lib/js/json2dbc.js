...
function generateTemplate(sortobj, obj){
    ...
    if(Array.isArray(obj))
    {
        for(let i = 0; i < arb_index_length; i++)
        {
            ...
            SG_ = ` SG_ ${bank_main} : ${Bytestart}|32@1+ (1,0) [${description2}] "${description3}" \r`;
            ...
            for(let j = 0; j < sg_index_length; j++)
            {
                ...
                sub_objdata["Tagname"] ? bank_sub = sub_objdata["Tagname"] : bank_sub = `${description}_Bank${Object.values(sortobj["arbid_index"])[i] + 1}`;
                SG_ = ` SG_ ${bank_sub} : ${bytestart}|32@1+ (1,0) [${description2}] "${description3}" \r`;
                template += SG_;
            }
        }
    }
