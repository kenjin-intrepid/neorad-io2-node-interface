...
function NativeConstructor(){
    ...
    EmitEvents.from.on('aout_reply', settings_reply => {
    ...
    returnObj.SetAoutValue = function(that, bank){
        ...
        {
            ExtraArray[0] = parseInt((channel1 * 65535) / 5);
        }
        if(channel2 > 0 && channel2 <= 5)
        {
            ExtraArray[1] = parseInt((channel2 * 65535) / 5);
        }
        if(channel3 > 0 && channel3 <= 5)
        {
            ExtraArray[2] = parseInt((channel3 * 65535) / 5);
        }
                ...
                for(let j = 0; j < 8; j++)
                {
                    ...
                    switch (deviceType) {
                        ...
                        case 4:
                            ...
                            $(`.device${i} .device_channel${ChipOnDevice} .neorad_aout_initinput_1`).val(initOutputValue1);
                            $(`.device${i} .device_channel${ChipOnDevice} .neorad_aout_initinput_2`).val(initOutputValue2);
                            $(`.device${i} .device_channel${ChipOnDevice} .neorad_aout_initinput_3`).val(initOutputValue3);
                            break;
                        case 5:
                            break;
                    }
        ...
        if(raw.State == neoRADIO2_RunStates.neoRADIO2state_Connected)
        {
            ...
            for (let i = 0; i < max_device; i++)
            {
                for(let j = 0; j < 8; j++)
                {
                    ...
                    let displayString;
                    let ChipOnDevice = j + 1;
                    let deviceType = raw['chainlist'][`device${i}`]['deviceType'];
                    let channelValue = raw['tempData'][`device${i}`][`channel${j}`];
                    if(raw['tempData'][`device${i}`])
                    {
                        switch (deviceType) {
                            case 0:
                                ...
                                temperature = temperatureConversion(channelValue);
                                ...
                                {
                                    if($(`.device${i} .device_channel${ChipOnDevice} input[data-check="${j+1}"]`).prop('checked')){
                                        $(`.device${i} .device_channel${ChipOnDevice} .ma_device_channel_temp`).html(def_lang.text_disc).removeClass("em15 online").addClass("em1 offline");
                                    }
                            ...
                            case 3:
                                ...
                                {
                                    temperature = voltageConversion(channelValue);
                                    unit = "mV";
                                ...
                                else if($(`input[name="ain_${i}_b${j+1}"]:checked`).val() == "high")
                                {
                                    ...
                                    {
                                        displayString = "Over Range";
                                        $(`.device${i} .device_channel${ChipOnDevice} .ma_device_channel_temp`).html(displayString).addClass('em15 online').removeClass("em1 offline");
    ...
    returnObj.saveCal = () => {
        ...
        for(let i = 0; i < tablelength; i++)
        {
            let tableOBJ = {
                ...
                bank7:[],
                bank8:[],
                deviceChannel: parseInt($($('.cal_th_header')[i]).attr('data-channel')),
                deviceRange: i
            };
