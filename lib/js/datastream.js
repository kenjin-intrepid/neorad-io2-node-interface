...
function NativeConstructor(){
        ...
        if(deviceConnected > 0)
        {
            for (let i = 0; i < deviceConnected; i++)
            {
                ...
                let deviceObject = {
                    ...
                    hardwareRev_major : returnObj.device_found['hardwareRev_major'][i],
                    hardwareRev_minor : returnObj.device_found['hardwareRev_minor'][i],
                };
    ...
    function reloadSettings(raw) {
        ...
        for(let i = 0; i < obj_length; i++)
        {
            ...
            let ReportRate = Object.values(objvalue)[i]['settingsReportRate'];
            ...
            $(`.device${deviceSave} .device_channel${i+1} .ma_device_channel_pullrate`).val(ReportRate);
            if( ReportRate == 10 ||
                ReportRate == 20 ||
                ReportRate == 50 ||
                ReportRate == 100 ||
            ...
            {
                $(`.device${deviceSave} .device_channel${i+1} .device_check_pullrate_select option[value="${ReportRate}"]`).prop('selected',true);
            }
            else
            {
                $(`.device${deviceSave} .device_channel${i+1} .device_check_pullrate_select option[value="0"]`).prop('selected',true);
                $(`.device${deviceSave} .device_channel${i+1} .ma_device_channel_pullrate`).prop('disabled',false);
                $(`.device${deviceSave} .device_channel${i+1} .ma_device_channel_pullrate`).parent().removeClass('opacity');
            }
