...
function NativeConstructor(){
        ...
        if(data_stream)
        {
            ...
            switch (data_stream["type"]) {
                ...
                case 3:
                    ...
                    $('.cal_table_tc_tables').append(append_cal_table(data_stream, AIN_range));
    ...
    EmitEvents.from.on('cal_inter', data_stream => {
        ...
        {
            change_status_text('Read error, please try again');
            $('.ma_calibration .input_selector_bg').click();
            return;
        }
        ...
        if(data_stream['tempData'] !== -1000 && data_stream['tempData'] != null)
        {
            ...
            switch (data_stream['type']) {
                case 0:
                    ...
                    final = parseFloat((first - parseFloat(data_stream['tempData'].toFixed(2))).toFixed(2));
                    break;
                case 3:
                    if(data_stream['range'] < 4)
                    {
                        $(document.activeElement).val(parseFloat(data_stream['tempData'].toFixed(4)) * 1000);
                        final = parseFloat((first - parseFloat(data_stream['tempData'].toFixed(4))).toFixed(5)) * 1000;
                    }
                    else
                    {
                        $(document.activeElement).val(parseFloat(data_stream['tempData'].toFixed(4)));
                        final = parseFloat((first - parseFloat(data_stream['tempData'].toFixed(4))).toFixed(5));
                    }
                    break;
                default:
                    $(document.activeElement).val(parseFloat(data_stream['tempData'].toFixed(2)));
                            ...
                            if (raw[usbKey][i] && enables)
                            {
                                switch (deviceType)
                                {
                                    case 0:
                                        ...
                                        {
                                            displayString = `${(measured_value.toFixed(2))}${TempUnit}`;
                                        }
        ...
        for(let i = 0; i < tablelength; i++)
        {
            ...
            for (let th_length_i = 0; th_length_i < th_length; th_length_i++)
            {
                ...
                for(let j = 1; j < 9; j++)
                {
                    ...
                    {
                        if(parseInt($('#device_serial option:selected').val()) == 3 && i < 4)
                        {
    ...
    returnObj.interactiveCal = bank => {
        ...
        if(parseInt($('#device_serial option:selected').val()) == 3)
        {
            ...
            tableOBJ.range = parseInt(AIN_range);
        }
...
$('.ma_device').on('click','.can_fill_apply',function () {
    ...
    if(isNaN(Startid))
    {
        change_status_text('Please enter a correct Arb ID');
        return;
    }
    ...
    switch (Value) {
        ...
        case "2":
            ...
            break;
        case "3":
            $(this).parents('.ma_device_wrap').find('.ma_device_can_type').val("2");
            $(this).parents('.ma_device_wrap').find('.ma_device_can_id').each(function (index, element) {
    ...
    for(let i = 0; i < ArbArray.length; i++)
    {
        for(let j = 0; j < ByteArray.length; j++)
        {
            if(ArbArray[i] == ArbArray[j] && i != j)
            {
                ...
                {
                    change_status_text('Export Failed, CAN ID overlap');
                    return;
                }
            }
        }
