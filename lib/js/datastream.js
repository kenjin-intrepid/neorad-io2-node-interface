...
function NativeConstructor(){
    ...
    try
    {
        Addon = require("bindings")("neoRAD_IO2");
    }
    catch(err)
    {
        console.log(err);
    ...
    {
        change_error_text("Native module can't be loaded， MSVC not installed?");
        terminateAll();
        $("#run_msvc").prop("disabled",false).removeClass("hidden");
        return;
    }
    ...
    EmitEvents.from.on('error_msg', msg => {
        switch (msg) {
            ...
            case "300":
                ...
                break;
            case "400":
                change_error_text('DOUT write settings failed');
                break;
            case "401":
                change_status_text('DOUT write settings applied');
                break;
            case "410":
                change_error_text('DIN change mode failed');
                break;
            case "411":
                change_status_text('DIN mode changed');
                break;
        }
    });
                    ...
                    switch (type)
                    {
                        ...
                        case 1:
                            ...
                            if(i < 5)
                            {
                                ...
                                // 1 = voltrig
                                // 2 = prescaler
                                eachBankSettings.channel1 = [mode1, parseInt(voltrig), isNaN(prescaler) ? 0 : prescaler];
                                eachBankSettings.channel2 = [mode2, parseInt(voltrig), isNaN(prescaler) ? 0 : prescaler];
                                eachBankSettings.channel3 = [mode3, parseInt(voltrig), isNaN(prescaler) ? 0 : prescaler];
                            }
                            else
                            {
                                let dout_prescaler = parseInt($(that).find(`.device_channel${i} .dout_prescaler`).val());
                                ...
                                // 0 = prescaler
                                eachBankSettings.channel1 = [isNaN(dout_prescaler) ? 0 : dout_prescaler];
                                eachBankSettings.channel2 = [isNaN(dout_prescaler) ? 0 : dout_prescaler];
                                eachBankSettings.channel3 = 1;
                            }
        ...
        for(let usbIndex = 0; usbIndex < usbDeviceLength; usbIndex++)
        {
            ...
            if(deviceState == 6)
            {
                ...
                for (let i = 0; i < deviceConnected; i++)
                {
                    ...
                    if(device_found[usbKey]['firmwareVersion_major'][i] !== 3 && device_found[usbKey]['firmwareVersion_minor'][i] !== 13)
                    {
                        need_update_arr.push(device_found[usbKey]['serialNumber'][i]);
                    }
                }
            }
            ...
            if(need_update_arr.length > 0)
            {
                need_update_arr.forEach(function (value){
                    console.log(`${value} firmware update needed`);
                })
            }
        }
                        ...
                        switch (deviceType)
                        {
                            case 1:
                                ...
                                else
                                {
                                    ...
                                    let settings1_prescale = raw[usbKey]['chainlist'][`device${i}`][`channel${j}`]['settings1']['prescale'];
                                    $(`.usb${usbIndex}_device${i} .device_channel${ChipOnDevice} .dout_prescaler`).val(settings1_prescale);
                            ...
                            if (raw[usbKey][i] && enables)
                            {
                                switch (deviceType)
                                {
                                    case 0:
                                        ...
                                        break;
                                    case 3:
                                        if ($(`input[name="ain_${usbIndex}_${i}_b${j}"]:checked`).val() == "low")
                                        {
                                        ...
                                        else if ($(`input[name="ain_${usbIndex}_${i}_b${j}"]:checked`).val() == "high")
                                        {
                                            measured_value = voltageConversion(channelValue);
                                            if (measured_value != undefined && measured_value != "d" && measured_value != "error")
                                            {
            ...
            switch (Object.values(objvalue)[i]['deviceType'])
            {
                case 1:
                    ...
                    else
                    {
                        ...
                        let settings1_prescale = Object.values(objvalue)[i]['settings1']['prescale'];
                        $(`.usb${deviceSaveUSB}_device${deviceSave} .device_channel${i+1} .dout_prescaler`).val(settings1_prescale);
    ...
    returnObj.sendDOUTsettings = (settings) => {
        EmitEvents.to.emit(12, JSON.stringify(settings));
    };

    returnObj.sendDINsettings = (settings) => {
        EmitEvents.to.emit(11, JSON.stringify(settings));
    };
