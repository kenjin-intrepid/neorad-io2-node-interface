...
function NativeConstructor(){
    ...
    EmitEvents.from.on('settings_reply_count', settings_reply_count => {
        change_status_text(`Writing in progress ${((parseInt(settings_reply_count) + 1) * 12.5).toFixed(0)}%`);
    });
        ...
        if(raw.State == neoRADIO2_RunStates.neoRADIO2state_Connected)
        {
            for (let i = 0; i < raw.maxID_Device; i++)
            {
                for(let j = 0; j < 8; j++)
                {
                    ...
                    $(`.ma_device .device${i} .device_channel${ChipOnDevice} .ma_device_can_id`).val(`0x${canArbid.toString(16).toUpperCase()}`);
                    ...
                    switch (deviceType) {
                        ...
                        case 4:
                            ...
                            {
                                $(`.ma_device .device${i} .device_channel${ChipOnDevice} .device_check_status1`).prop('checked',true);
                                $(`.ma_device .device${i} .device_channel${ChipOnDevice} .ma_device_channel_status1`).text(def_lang.text_enable);
                                $(`.ma_device .device${i} .device_channel${ChipOnDevice} .neorad_aout_input_1`).prop('disabled',false);
                            }
                            else
                            {
                                $(`.ma_device .device${i} .device_channel${ChipOnDevice} .device_check_status1`).prop('checked',false);
                                $(`.ma_device .device${i} .device_channel${ChipOnDevice} .ma_device_channel_status1`).text(def_lang.text_disable);
                                $(`.ma_device .device${i} .device_channel${ChipOnDevice} .neorad_aout_input_1`).prop('disabled',true);
                            }

                            if(settings2_enable > 0)
                            {
                                $(`.ma_device .device${i} .device_channel${ChipOnDevice} .device_check_status2`).prop('checked',true);
                                $(`.ma_device .device${i} .device_channel${ChipOnDevice} .ma_device_channel_status2`).text(def_lang.text_enable);
                                $(`.ma_device .device${i} .device_channel${ChipOnDevice} .neorad_aout_input_2`).prop('disabled',false);
                            }
                            else
                            {
                                $(`.ma_device .device${i} .device_channel${ChipOnDevice} .device_check_status2`).prop('checked',false);
                                $(`.ma_device .device${i} .device_channel${ChipOnDevice} .ma_device_channel_status2`).text(def_lang.text_disable);
                                $(`.ma_device .device${i} .device_channel${ChipOnDevice} .neorad_aout_input_2`).prop('disabled',true);
                            }

                            if(settings3_enable > 0)
                            {
                                $(`.ma_device .device${i} .device_channel${ChipOnDevice} .device_check_status3`).prop('checked',true);
                                $(`.ma_device .device${i} .device_channel${ChipOnDevice} .ma_device_channel_status3`).text(def_lang.text_enable);
                                $(`.ma_device .device${i} .device_channel${ChipOnDevice} .neorad_aout_input_3`).prop('disabled',false);
                            }
                            else
                            {
                                $(`.ma_device .device${i} .device_channel${ChipOnDevice} .device_check_status3`).prop('checked',false);
                                $(`.ma_device .device${i} .device_channel${ChipOnDevice} .ma_device_channel_status3`).text(def_lang.text_disable);
                                $(`.ma_device .device${i} .device_channel${ChipOnDevice} .neorad_aout_input_3`).prop('disabled',true);
                            }
    ...
    function reloadSettings(raw) {
        ...
        for(let i = 0; i < Object.keys(objvalue).length; i++)
        {
            ...
            if(Object.values(objvalue)[i]['deviceType'] == 4)
            {
                ...
                if(settings1_enable > 0)
                {
                    $(`.ma_device .device${deviceSave} .device_channel${i+1} .device_check_status1`).prop('checked',true);
                    $(`.ma_device .device${deviceSave} .device_channel${i+1} .ma_device_channel_status1`).text(def_lang.text_enable);
                    $(`.ma_device .device${deviceSave} .device_channel${i+1} .neorad_aout_input_1`).prop('disabled',false);
                }
                else
                {
                    $(`.ma_device .device${deviceSave} .device_channel${i+1} .device_check_status1`).prop('checked',false);
                    $(`.ma_device .device${deviceSave} .device_channel${i+1} .ma_device_channel_status1`).text(def_lang.text_disable);
                    $(`.ma_device .device${deviceSave} .device_channel${i+1} .neorad_aout_input_1`).prop('disabled',true);
                }

                if(settings2_enable > 0)
                {
                    $(`.ma_device .device${deviceSave} .device_channel${i+1} .device_check_status2`).prop('checked',true);
                    $(`.ma_device .device${deviceSave} .device_channel${i+1} .ma_device_channel_status2`).text(def_lang.text_enable);
                    $(`.ma_device .device${deviceSave} .device_channel${i+1} .neorad_aout_input_2`).prop('disabled',false);
                }
                else
                {
                    $(`.ma_device .device${deviceSave} .device_channel${i+1} .device_check_status2`).prop('checked',false);
                    $(`.ma_device .device${deviceSave} .device_channel${i+1} .ma_device_channel_status2`).text(def_lang.text_disable);
                    $(`.ma_device .device${deviceSave} .device_channel${i+1} .neorad_aout_input_2`).prop('disabled',true);
                }

                if(settings3_enable > 0)
                {
                    $(`.ma_device .device${deviceSave} .device_channel${i+1} .device_check_status3`).prop('checked',true);
                    $(`.ma_device .device${deviceSave} .device_channel${i+1} .ma_device_channel_status3`).text(def_lang.text_enable);
                    $(`.ma_device .device${deviceSave} .device_channel${i+1} .neorad_aout_input_3`).prop('disabled',false);
                }
                else
                {
                    $(`.ma_device .device${deviceSave} .device_channel${i+1} .device_check_status3`).prop('checked',false);
                    $(`.ma_device .device${deviceSave} .device_channel${i+1} .ma_device_channel_status3`).text(def_lang.text_disable);
                    $(`.ma_device .device${deviceSave} .device_channel${i+1} .neorad_aout_input_3`).prop('disabled',true);
                }
            }
        }
