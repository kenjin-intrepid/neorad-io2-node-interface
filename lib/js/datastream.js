        ...
        for(let usbIndex = 0; usbIndex < usbDeviceLength; usbIndex++)
        {
            ...
            if(deviceConnected > 0)
            {
                for (let i = 0; i < deviceConnected; i++)
                {
                    ...
                    $(".ma_device .table-responsive-sm").append(`<div class="ma_device_wrap usb${usbIndex} usb${usbIndex}_device${i}" data-type="${deviceObject.deviceType}" data-number="${i+1}">${AppendTemplate}</div>`);
                }
            }
        }
