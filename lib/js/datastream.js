...
function NativeConstructor(){
    ...
    EmitEvents.from.on('device_found', device_found => {
        ...
        if(device_found != "null" && device_found_once === 0)
        {
            ...
            showConnected(returnObj.device_found["usb0"]);
            readSettings(returnObj.device_found["usb0"]);
            device_found_once = 1;
    ...
    EmitEvents.from.on('data_stream', data_stream => {
        ...
        if(start_get_data)
        {
            processRawData((JSON.parse(data_stream))["usb0"]);
        }
    });
    ...
    function showConnected(device_found){
        let deviceConnected = device_found.maxID_Device;
        ...
        if(deviceConnected > 0)
        {
            ...
            {
                devices[i] = {
                    deviceType : device_found['chainlist'][`device${i}`]['channel0']['deviceType'],
                    serialNumber : device_found['serialNumber'][i],
                };
                ...
                let deviceObject = {
                    deviceType : device_found['chainlist'][`device${i}`]['channel0']['deviceType'],
                    serialNumber : device_found['serialNumber'][i],
                    manufacture_year : device_found['manufacture_year'][i],
                    manufacture_month : device_found['manufacture_month'][i],
                    manufacture_day : device_found['manufacture_day'][i],
                    firmwareVersion_major : device_found['firmwareVersion_major'][i],
                    firmwareVersion_minor : device_found['firmwareVersion_minor'][i],
                    hardwareRev_major : device_found['hardwareRev_major'][i],
                    hardwareRev_minor : device_found['hardwareRev_minor'][i],
                };
