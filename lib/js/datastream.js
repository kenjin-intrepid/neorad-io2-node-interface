...
function NativeConstructor(){
    ...
    EmitEvents.from.on('cal_inter', data_stream => {
        ...
        if(me_data !== -1000 && me_data != null)
        {
            ...
            switch (data_stream['type']) {
                case 0:
                    $(document.activeElement).val(parseFloat(me_data.toFixed(2)));
                    final = parseFloat((first - me_data).toFixed(2));
                    break;
                case 3:
...
$('.ma_device').on('click','.can_fill_apply',function () {
    ...
    $(this).parents('.ma_device_wrap').find('.ma_device_can_type').val(cantype_set);
    $(this).parents('.ma_device_wrap').find('.ma_device_can_id_type').val(cantypeid_set);
