...
function NativeConstructor(){
    ...
    myPath = path.join(mypath, `IntrepidCS\/RAD-IO2\/${DateString}.csv`);
    ...
    returnObj.startDataPush = () => {
        ...
        if(writeStream === 0)
        {
            ...
            {
                writeStream = fs.createWriteStream( path.join(mypath, `IntrepidCS\/RAD-IO2\/auto.csv`), {"flags":"w"});
            }
        }
    };
    ...
    returnObj.gatherData = function(that){
        ...
        if(!$(that).find(`.device_channel1 .ma_device_can_type`).val())
        {
            $('.device_save').prop('disabled',false);
            $('.device_save_canhub').prop('disabled',false);
            $('.device_default').prop('disabled',false);
            $('#device_startlog').prop('disabled',false);
            $('html,body').removeClass('wait');
            change_error_text('Please check CAN settings');
            return
        }

        if(!$(that).find(`.device_channel1 .ma_device_can_id_type`).val())
        {
            $('.device_save').prop('disabled',false);
            $('.device_save_canhub').prop('disabled',false);
            $('.device_default').prop('disabled',false);
    ...
    returnObj.getCanSettings = function(that){
        ...
        for (let i = 0; i < iloop; i++)
        {
            ...
            else
            {
                ...
                can_settings["data"][i]['dout_mode_ch1'] = $(that).find(`.device_channel${i+1} .dout_mode_select_1`).val();
                can_settings["data"][i]['dout_mode_ch2'] = $(that).find(`.device_channel${i+1} .dout_mode_select_2`).val();
            }
                    ...
                    for (let j = 0; j < 8; j++)
                    {
                        ...
                        switch (canMsgType)
                        {
                            ...
                            case 255:
                                canType = 1;
                                canIdType = 4;
                                if(canArbid == 0xFFFFFFFF)
                                {
                                    change_error_text('Please check CAN ID settings');
                                }
                                break;
                        }
                    ...
                    if(deviceType == 0 || deviceType == 3)
                    {
                        for (let j = 1; j < 9; j++)
                        {
                            ...
                            if (raw[usbKey][i] && enables)
                            {
                                ...
                                {
                                    let time_us = (time_running / 1000).toFixed(1);
                                    ...
                                    if(len > 80000)
                                    {
                                        ...
                                        if (autoSave)
                                        {
                                            ...
                                            let Path = path.join(mypath, `IntrepidCS\/RAD-IO2\/PlotHistory\/${month}-${day} ${startTime}-${end}.json`);
                            ...
                            if (measured_value === temperatureString)
                            {
                                rawObj['time'] = (time_running / 1000).toFixed(1);
...
$('.ma_device').on('click','.set_export',function(){
    ...
    let myPath = path.join(mypath, `IntrepidCS\/RAD-IO2\/${fileNameType}.json`);
...
$('.ma_device').on('click','.can_export',function(){
    ...
    let myPath = path.join(mypath, `IntrepidCS\/RAD-IO2\/${file}.dbc`);
