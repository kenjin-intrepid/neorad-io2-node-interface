...
function addRow(Parent) {
    ...
    switch ($('#device_serial').val())
    {
        case "0" :
            ...
            break;
        case "3" :
            $($(parent).find('tbody')).append(`${cal_row_temp($('#device_serial option:selected').val(),null,null, addRow.caller.name, parseInt($('.cal_ain_range .ain_select option:selected').val()) - 1)}`);
            break;
        default:
            break;
    }
...
function saveJSON(call) {
    ...
    for(let i = 0; i < tablelength; i++)
    {
        ...
        else if(tablelength == 8)
        {
            ...
            let rangeNumber = parseInt($($(".cal_table_")[i]).attr('data-bank'));
            let bankTitle = "";
            switch (rangeNumber)
            {
                case 0:
            ...
            for(let j = 0; j < rowlength; j++)
            {
                ...
                for(let k = 1; k < 4; k++)
                {
                    ...
                    tableOBJ[bankTitle][a][`bank${k}`] = point_value;
                }
            }
        }
    }
    ...
    let myPath = path.join(mypath, `IntrepidCS\/RAD-IO2\/${fileName}.json`);
...
function readJSON() {
    ...
    let myPath = path.join(mypath, `IntrepidCS\/RAD-IO2\/${serial}.json`);
    ...
    dialog.showOpenDialog({
        ...
        fs.readFile(fileName, 'utf8', function (err, data) {
            ...
            {
                change_error_text("The file you're trying to load has a different serial");
                return
            }
                ...
                case 4:
                    ...
                    for(let  i = 0; i < 8; i++)
                    {
                        ...
                        switch (cal_value)
                        {
                            ...
                            case "Bank7":
                                position = 6;
                                data_stream["data"][6]["calpoints"] = Object.keys(Object.values(dataObj["data"])[i]);
                                break;
                            case "Bank8":
                                position = 7;
                                data_stream["data"][7]["calpoints"] = Object.keys(Object.values(dataObj["data"])[i]);
                                break;
                            default:
                                console.log("error");
                                break
            ...
            switch (data_stream["type"]) {
                ...
                case 4:
                    $('.cal_table_tc_tables').html('');
                    $('.cal_table_tc_tables').append(import_cal_table_aout(data_stream));
                    $('.cal_table_').removeClass("hidden");
                    difference();
                    break;
                default:
