...
function cal_row_temp(type, data, index, fCall, data_index) {
    ...
    switch (type)
    {
        ...
        case "3":
            if(cal_index && cal_index != "0")
            {
                ...
                cal_index = parseFloat(cal_index.toFixed(6));
            }

            if(data_index < 3)
            {
                tdh_extra = `<input type="number" class="cal_input_th" value="${cal_index * 1000}" ${cal_index ? "disabled" : "enabled"}> mV`;
            }
            else
            {
    ...
    for(let i = 1; i < 9; i++)
    {
        ...
        if(data !== null && index !== null)
        {
            ...
            let length = Math.floor(parseFloat(data['datapoints'][i-1][index])).toString().length;
            let TC_precision = length + 1;

            switch (type)
            {
                case "0":
                    value = (parseFloat(data['datapoints'][i-1][index]) * 1000).toPrecision(TC_precision);
                    break;
                case "3":
                    if(data_index < 3)
                    {
                        value = (parseFloat(data['datapoints'][i-1][index]) * 1000).toPrecision(6);
                    }
                    else
                    {
                        value = (parseFloat(data['datapoints'][i-1][index])).toPrecision(6);
                    }
                    break;
            }
        }
...
function append_cal_table(data) {
    ...
    function tableTemp(cal_rows, index){
        return `
        <table class="table table-striped cal_table_ hidden" id="cal_table_${index + 1}" data-type="${data['type']}">
            <thead class="thead-dark">
            <tr>
...
function cal_row_aout_temp(data, index, fCall) {
    ...
    let tdh_extra = `<input type="number" class="cal_input_th" value="${cal_index}" ${cal_index ? "disabled" : "enabled"}> V`;
    ...
    for(let i = 1; i < 4; i++)
    {
        ...
        <td>
            <div class="flex">
                <input class="cal_input cal_stored" disabled placeholder="${def_lang.cal_stored}" value="${value}">
                <input class="cal_input cal_me" placeholder="Enter Here" data-channel="${i}" value="${value}">
                <input class="cal_input cal_diff" disabled>
            </div>
        </td>
    `;
