...
function cal_row_temp(type, data, index, fCall, data_index) {
    ...
    switch (type)
    {
        ...
        case "3":
            ...
            if(data_index < 3)
            {
                tdh_extra = `<input type="number" class="cal_input_th" value="${(cal_index * 1000).toFixed(floatPoints)}" ${cal_index ? "disabled" : "enabled"}> mV`;
            }
            else
            {
    ...
    for(let i = 1; i < 9; i++)
    {
        ...
        if(data !== null && index !== null)
        {
            ...
            if(type == "3" && data_index < 3)
            {
                value = (parseFloat(data['datapoints'][i-1][index]) * 1000).toFixed(floatPoints);
            }
            else
            {
