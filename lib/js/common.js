...
function can_validation() {
    ...
    {
        let id_node = Array.from($(`.ma_device .device${i} .ma_device_can_id`));
        let byte_node = Array.from($(`.ma_device .device${i} .ma_device_can_byte`));

        for(let j = 0; j < id_node.length; j++)
        {
            for(let k = 0; k < id_node.length; k++){
...
function change_status_text(str) {
    ...
    $('.statustext span').delay(3000).fadeOut(1000);
}
        ...
        fs.readFile(fileName, 'utf8', function (err, data) {
            ...
            switch (data_stream.type) {
                case 0:
                    ...
                    for(let  i = 0; i < Object.keys(dataObj["data"]).length; i++)
                    {
                ...
                case 3:
                    ...
                    for(let  i = 0; i < 6; i++)
                    {
                        ...
                        for(let l = 0; l < 8; l++)
                        {
                            for(let k = 0; k < Object.keys(Object.values(dataObj["data"])[i]).length; k++)
                            {
