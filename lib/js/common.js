...
$('.ma_device').on('blur','.ma_device_can_byte',function () {
    ...
    let canid = $(this).parents('td').find('.ma_device_can_id').val();
    ...
    {
        let canid_array_value = $($(this).parents('tr').find('.ma_device_can_id')[i]).val();
...
function change_status_text(str) {
    ...
    $('.statustext span').delay(4000).fadeOut(1000,function () {
        $('.navbar_items').show();
    });
}