...
function DIO_enable(){
    $('.dout_set_apply').prop('disabled',false);
}

function DIO_disable(){
    $('.dout_set_apply').prop('disabled',true);
}

$("#run_msvc").on("click", function (){
    Child_Process(msvc_path, function (err, data) {
        if(err)
        {
            console.log(err)
        }
        ...
        $("#run_msvc").prop("disabled",true).addClass("hidden");
    });
})