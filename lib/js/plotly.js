...
function chartSeriesInit() {
    ...
    let temparray = [];
    for (let j = 0; j < PlotSeries.maxD; j++)
    {
        temparray[j] = [];
        for (let i = 1; i < 9; i++)
        {
...
function clickStart()
{
    ...
    {
        refresh_rate = settings.get('rate');
    }
...
function getChannels() {
    ...
    {
        throw "RAD-IO2 not connected";
    }

    $(".ma_graph .ma_graph_sidebar .ma_graph_sidebar_inner").html("");

    let usbDeviceLength = Object.keys(globalObj.device_found).length;
    for(let usbIndex = 0; usbIndex < usbDeviceLength; usbIndex++)
    {
        let usbKey = `usb${usbIndex}`;
        let deviceConnected = globalObj.device_found[usbKey].maxID_Device;
        let deviceState = globalObj.device_found[usbKey].State;
        if(deviceState == 5)
        {
            for (let i = 0; i < deviceConnected; i++)
            {
                let deviceObject = {
                    deviceType : globalObj.device_found[usbKey]['chainlist'][`device${i}`]['channel0']['deviceType'],
                    serialNumber : globalObj.device_found[usbKey]['serialNumber'][i],
                    enables : globalObj.device_found[usbKey]['chainlist'][`device${i}`]
                };
                let AppendTemplate = template2(deviceObject,usbIndex,i);
...
function selectHistoryPlot() {
    ...
    {
        let Path = path.join(mypath, `IntrepidCS\/RAD-IO2\/PlotHistory\/${Value}.json`);
...
function getAllPlots() {
    let Path = path.join(mypath, `IntrepidCS\/RAD-IO2\/PlotHistory`);
